  ...
  return (
    ...
    <section className="select-none dark:bg-gray-900 overflow-hidden w-full h-screen min-h-screen">
      <button
       ...
       className='text-white flex opacity-80 absolute left-5 top-3 hover:opacity-100 items-center'>
      ...
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        onSubmit={async values => {
          if(values.companyCode){
           try {
            setLoading(true);
            const {data} = await axios.post(`${CompanyBaseApi}/create`,{companyCode:values.companyCode},Apiconfig);
            if(data && data.company){
                const newCompany = data.company as Company;
                setLoading(false);
                Navigate(`/${newCompany._id}/Projects`);
            }
             setLoading(false);  
           } catch (error:any) {
            setLoading(false);
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
          <div>
           ...
           type="text"
              ...
              id="companyCode"
              placeholder="Enter the Company's code"
              className={`${ errors.companyCode && touched.companyCode ? "border-red-500 outline-red-500":''} bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
              autoComplete='off'
            />
 
 {!errors.companyCode  && loginError && loginError!='' && (
              <div className="text-sm text-red-500 font-bold">{error}</div>
            )}

             {errors.companyCode && touched.companyCode && (
              <div className="text-sm text-red-500 font-bold">{errors.companyCode}{`*`}</div>
            )}
          </div>
          ...
          >
           <span className='sm:text-base text-xs'> Join Company</span>
