...
const JoinedCompanies = () => {
    ...
    const [joinedCompanies,setjoinedCompanies]=useState<Company[] | []>([]);
  ...
  const fetchCompaniesJoined = async()=>{
    ...
    try {
        ...
        const {data} = await axios.post(`${CompanyBaseApi}/joinedCompanies`,{search:search},Apiconfig);
        if(data && data.companiesJoined){
            const FetchedjoinedCompanies = data.companiesJoined as Company[];
            setjoinedCompanies(FetchedjoinedCompanies);
        }
...
const deleteYourCompany = async(companyId:string)=>{
    ...
    try {
        setCompanyLoading(true);
        const {data} = await axios.post(`${CompanyBaseApi}/joinedCompanies/delete`,{companyId:companyId},Apiconfig);
        if(data){
            await fetchCompaniesJoined();
        }
        setCompanyLoading(false);
    } catch (error) {
        setCompanyLoading(false);
       ...
       transition-all duration-500 transform focus:w-[170px]  ease-in-out rounded-md px-2 outline-none placeholder:text-sm  text-base"
        ...
        joinedCompanies.map((company)=>(
            ...
            key={company._id} className={`${companyLoading?'select-none animate-pulse':''} flex justify-between min-w-full bg-secondary items-center p-5`}>
             ...
             }} className='hover:underline cursor-pointer flex gap-5'> 
             <span>{company.name}</span>
             </button>
             <div className='flex gap-5 items-center'>
             <span className='text-sm opacity-80 font-light'>Created on : {new Date(company.createdAt).toDateString()}</span>
             <span className='text-sm opacity-80 font-light'>{user && `Role: ${company.members.find(member=>member.user===user._id)?.role}`}</span> 
              </div>
             </div>
             <button 
             disabled={companyLoading}
